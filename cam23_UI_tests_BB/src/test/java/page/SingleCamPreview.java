...
public abstract class SingleCamPreview extends ModeMenu {
	...
	/** 
	 *  Capture the xTimes amount of single images**/
	public static void capture(int xTimes, int mSecBetweenCaptures) throws InterruptedException{
		...
		if (filesCaptured == xTimes)
			System.out.printf("\n\t'%d' Single Shot images were captured out of the specified number of '%d'!\n", filesCaptured, xTimes);
		else {
			System.out.printf("\n\t'%d' Single Shot images were captured out of the specified number of '%d'.  Capture failed!!\n", filesCaptured, xTimes);
			assertTrue(false);
		}
	}
    ...
    public static void loaded() throws InterruptedException {
    	...
    	assertTrue(Helpers.getCameraMode() == "single");
    }	
}